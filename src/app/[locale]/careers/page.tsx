...
function RoleCard({
  ...
  return (
    <article className="group flex flex-col gap-4 border border-line p-8 transition-all duration-300 hover:border-navy hover:shadow-sm">
      <div>
        <p className="text-[8px] font-bold tracking-[0.22em] text-gold-500 uppercase mb-2">{type}</p>
      ...
      <button
        onClick={onApply}
        className="group/btn inline-flex items-center gap-2 self-start text-[9px] font-bold tracking-[0.22em] text-navy uppercase transition-colors hover:text-gold-500"
      >
        {applyLabel}
      ...
      <main className="bg-white min-h-screen">
          ...
          <Container>
            <Reveal>
              <div className="mb-12 flex items-end border-b border-line pb-4 justify-between">
                ...
                <span className="text-[9px] font-bold tracking-widest text-charcoal/30 uppercase">
                  {ROLES.length} Open
                </span>
              </div>
            </Reveal>
              ...
              {ROLES.map((r, i) => (
                <Reveal key={r} delay={i * 60}>
                  <RoleCard
                    ...
                    type={t(`${r}Type` as Parameters<typeof t>[0])}
                    desc={t(`${r}Desc` as Parameters<typeof t>[0])}
            ...
            <Reveal className="mt-20">
              <div className="border border-line bg-navy p-10 md:p-14 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                <div className="flex flex-col gap-2 max-w-lg">
                  <p className="text-[8px] font-bold tracking-[0.3em] text-gold-500 uppercase">{t('applyHeading')}</p>
                  <p className="text-[15px] text-white/60 leading-relaxed">{t('applySub')}</p>
                </div>
                <button
                  onClick={() => setApplyOpen(true)}
                  className="shrink-0 border border-gold-500 px-8 py-3 text-[9px] font-bold tracking-[0.22em] text-gold-500 uppercase transition-colors hover:bg-gold-500 hover:text-navy"
                >
                  {t('applyBtn')}
                </button>
              </div>
        ...
        {applyOpen && (
          <div
            ...
            onClick={(e) => { if (e.target === e.currentTarget) { setApplyOpen(false); setSubmitted(false); } }}
          >
            <div className="relative w-full max-w-xl bg-white p-8 md:p-12 shadow-2xl max-h-[90vh] overflow-y-auto">
              <button
              ...
              {submitted ? (
                ...
                <>
                  <p className="text-[8px] font-bold tracking-[0.3em] text-gold-500 uppercase mb-2">{t('eyebrow')}</p>
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    ...
                    <button
                      type="submit"
                      className="bg-navy py-4 text-[9px] font-bold tracking-[0.28em] text-white uppercase transition-colors hover:bg-navy/85"
                    >
                      {t('applySubmit')}
                    </button>
                  </form>
